...
let client = new OSS({
    ...
    accessKeyId: 'LTAI4GF8DpWaXyfAPTBPR7Hn',
    accessKeySecret: 'YLoI0kSx5QqmDiRmCzLtLbGKmh6WBY',
    bucket: 'rwenjie-blog'
});
...
export const put = async (ObjName, fileUrl) => {
    try {
        const url = 'diy-shop/'+this.$store.state.profile.username+'/'+ObjName;
        let result = await client.put(`${url}`, fileUrl);
